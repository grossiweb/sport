...
import { useMostBetMatchups } from '@/hooks/useMostBetMatchups'
    ...
    <div className="space-y-4">
      ...
      {isLoading || isFetching ? (
        ...
        <div className="text-sm text-red-600 dark:text-red-400">
          Failed to load most bet matchups.
        </div>
      ) : !items || items.length === 0 ? (
        <div className="text-sm text-gray-600 dark:text-gray-400">
          No upcoming games with betting lines found for this window.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {items.map(({ game, consensusSpread }) => (
            <MostBetGameCard
              key={game.id}
              game={game}
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900/20 rounded-lg">
            <div className="flex items-center space-x-3">
              <TeamLogo team={game.awayTeam} size="sm" className="flex-shrink-0" />
              <div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {game.awayTeam.record && <span className="ml-2">({game.awayTeam.record})</span>}
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900/20 rounded-lg">
            <div className="flex items-center space-x-3">
              <TeamLogo team={game.homeTeam} size="sm" className="flex-shrink-0" />
              <div>
                <div className="font-medium text-gray-900 dark:text-white">
                  {game.homeTeam.name}
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {game.homeTeam.record && <span className="ml-2">({game.homeTeam.record})</span>}
                </div>
              </div>
            </div>
            <div className="text-right">
